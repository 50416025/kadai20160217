...
public class ReservationControl {
	...
	//指定した施設の概要
		public String facilityInfomation( String facility){
			String res = "";
			...
			try {
				// 予約情報を取得するクエリ
				ResultSet rs = mysql.getInformation(facility);
				boolean exist = false;
				while(rs.next()){
					String start = rs.getString("open_time");
					String end = rs.getString("close_time");
					String info = rs.getString("explanation");
					res += " " + start + " -- " + end + " -- "  + info + "\n";
					exist = true;
				}
		...
		//施設の予約状況（全体）
		public String getReservationAll(String facility){
			String res = "";
			...
			try {
				// 予約情報を取得するクエリ
				ResultSet rs = mysql.getReservationAll(facility);
				boolean exist = false;
				while(rs.next()){
...
//////予約のキャンセル
	public String deleteReservation(MainFrame frame){
		...
		if ( flagLogin){ // ログインしていた場合
			//新規予約画面作成
			DeleteDialog rd = new DeleteDialog(frame);
							...
							ResultSet rs = mysql.selectReservation(rdate, facility);
						      ...
						      while(rs.next()){
							        ...
							        if( (start.compareTo(st)<=0 && st.compareTo(start)<=0) ||		//レコードの開始時刻＜新規の開始時刻　AND　新規の開始時刻＜レコードの終了時刻
							        	 (st.compareTo(end)<0 && start.compareTo(et)<0)){		//新規の開始時刻＜レコードの開始時刻　AND　レコードの開始時刻＜新規の開始時刻
										 	// 重複有りの場合に ng をtrueに設定
							        	ng = true; break;
							        }
						      }
						      ...
						      if (!ng){	//重なっていた場合
						    	  ...
						    	  int rs_int = mysql.deleteReservation(rdate, st, et, res, facility);
						    	  res = "予約をキャンセルしました";
						      }
						}catch (Exception e) {
							e.printStackTrace();
